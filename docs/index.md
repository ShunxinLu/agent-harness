# Harness Docs Index

Last updated: 2026-03-03

## Active plans

1. [Harness Team Implementation Plan v2](./harness-team-implementation-plan-v2.md)
2. [Harness Implementation Task Sequence](./harness-implementation-task-sequence.md)
3. [Harness Consolidation Implementation Plan](./harness-consolidation-implementation-plan.md)

## Architecture

1. [Harness Architecture Design and OSS Replacement Assessment](./harness-architecture-design.md)
2. [Harness Service Architecture (Multi-User Mode)](./harness-service-architecture.md)

## Legacy references

1. [Harness Team Implementation Plan (v1)](./harness-team-implementation-plan.md)
